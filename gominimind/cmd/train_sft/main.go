...
func main() {
	...
	var (
		...
		saveWeight        = flag.String("save_weight", "full_sft", "保存权重的前缀名")
		epochs            = flag.Int("epochs", 6, "训练轮数")
		batchSize         = flag.Int("batch_size", 16, "batch size")
		learningRate      = flag.Float64("learning_rate", 1e-6, "初始学习率（SFT使用很小学习率）")
		...
		dataPath          = flag.String("data_path", "../dataset/sft_512.jsonl", "SFT数据路径")
		fromWeight        = flag.String("from_weight", "pretrain", "基于哪个权重训练（建议从预训练权重开始）")
	...
	flag.Usage = func() {
		...
		flag.PrintDefaults()
		fmt.Fprintln(os.Stderr, "\n示例:")
		fmt.Fprintln(os.Stderr, "  从预训练权重开始SFT微调:")
		fmt.Fprintln(os.Stderr, "    go run cmd/train_sft/main.go -from_weight pretrain")
		fmt.Fprintln(os.Stderr, "  从检查点恢复训练:")
		fmt.Fprintln(os.Stderr, "    go run cmd/train_sft/main.go -from_weight pretrain -from_resume 1")
		fmt.Fprintln(os.Stderr, "  使用自定义参数:")
		fmt.Fprintln(os.Stderr, "    go run cmd/train_sft/main.go -batch_size 8 -learning_rate 4e-7")
	}
	...
	trainer.Logger("========================================")
	trainer.Logger("MiniMind Go SFT微调启动")
	trainer.Logger("========================================")
	...
	if err != nil {
		trainer.Logger("创建SFT模型失败: %v", err)
		os.Exit(1)
	}

	// 创建训练器
	t, err := trainer.NewSFTTrainer(cfg, m, tok)
	if err != nil {
		trainer.Logger("创建SFT训练器失败: %v", err)
		os.Exit(1)
	}

	// 开始训练
	if err := t.Train(); err != nil {
		trainer.Logger("SFT训练失败: %v", err)
		os.Exit(1)
	}
