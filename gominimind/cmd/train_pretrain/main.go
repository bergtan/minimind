...
func main() {
	...
	var (
		...
		saveWeight        = flag.String("save_weight", "pretrain", "保存权重的前缀名")
		epochs            = flag.Int("epochs", 1, "训练轮数（建议1轮zero或2-6轮充分训练）")
		batchSize         = flag.Int("batch_size", 32, "batch size")
		learningRate      = flag.Float64("learning_rate", 5e-4, "初始学习率")
		device            = flag.String("device", "cpu", "训练设备 (cpu)")
		numWorkers        = flag.Int("num_workers", 4, "数据加载线程数")
		accumulationSteps = flag.Int("accumulation_steps", 8, "梯度累积步数")
		gradClip          = flag.Float64("grad_clip", 1.0, "梯度裁剪阈值")
		logInterval       = flag.Int("log_interval", 100, "日志打印间隔")
		saveInterval      = flag.Int("save_interval", 1000, "模型保存间隔")
		...
		dataPath          = flag.String("data_path", "../dataset/pretrain_hq.jsonl", "预训练数据路径")
		fromWeight        = flag.String("from_weight", "none", "基于哪个权重训练，为none则从头开始")
	...
	flag.Usage = func() {
		...
		flag.PrintDefaults()
		fmt.Fprintln(os.Stderr, "\n示例:")
		fmt.Fprintln(os.Stderr, "  从头开始预训练:")
		fmt.Fprintln(os.Stderr, "    go run cmd/train_pretrain/main.go")
		fmt.Fprintln(os.Stderr, "  从检查点恢复训练:")
		fmt.Fprintln(os.Stderr, "    go run cmd/train_pretrain/main.go -from_resume 1")
		fmt.Fprintln(os.Stderr, "  使用自定义参数:")
		fmt.Fprintln(os.Stderr, "    go run cmd/train_pretrain/main.go -batch_size 16 -learning_rate 1e-4")
	}
	...
	trainer.Logger("========================================")
	trainer.Logger("MiniMind Go 预训练启动")
	trainer.Logger("========================================")
	...
	trainer.Logger("  学习率: %.6f", cfg.LearningRate)
	trainer.Logger("  梯度累积: %d", cfg.AccumulationSteps)
	trainer.Logger("  梯度裁剪: %.2f", cfg.GradClip)
	...
	if err != nil {
		trainer.Logger("创建模型失败: %v", err)
		os.Exit(1)
	}

	// 加载已有权重（如果指定）
	if cfg.FromWeight != "none" {
		modelPath := filepath.Join(cfg.SaveDir, cfg.FromWeight+".bin")
		if err := m.Load(modelPath, modelConfig); err != nil {
			trainer.Logger("加载权重失败: %v，将从头开始训练", err)
		} else {
			trainer.Logger("成功加载权重: %s", cfg.FromWeight)
		}
	}

	// 创建训练器
	t, err := trainer.NewPretrainTrainer(cfg, m, tok)
	if err != nil {
		trainer.Logger("创建训练器失败: %v", err)
		os.Exit(1)
	}

	// 开始训练
	if err := t.Train(); err != nil {
		trainer.Logger("训练失败: %v", err)
		os.Exit(1)
	}
